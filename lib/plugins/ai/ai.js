...
.defines(function(){
    ig.ai = ig.Class.extend({ 
        ...
        getAction: function(entity){
            ...
            var playerList= ig.game.getEntitiesByType('EntityTankplayer');
  ...
  defines(function(){
	...
	doAction: function(action){
	  ...
	  if(res.collision.y){
			...
			}else{return this.doAction(ig.ai.ACTION.MoveRight);}
	  }
	  if(distance < 100){
	      //decide between attacking, blocking or just being lazy //
				var decide = Math.random();
				console.log(decide);
	      if(decide <= 0.3){return this.doAction(ig.ai.ACTION.MoveRight);}
	      if(decide > 0.3 && decide < 0.4){return this.doAction(ig.ai.ACTION.Attack);}
	      return this.doAction(ig.ai.ACTION.MoveDown);
	  }
	  if( distance > 100 && distance < 300) {
			//console.log(distance)
			//console.log(angle)
			var pi4 = Math.PI / 4;   //0.78539  3*pi4=2.35619
	    //	if you can walk in a straight line go for it
	    if(Math.abs(angle) < pi4){ return this.doAction(ig.ai.ACTION.MoveRight); }
	    if(Math.abs(angle) >  3 * pi4) {return this.doAction(ig.ai.ACTION.MoveLeft);}
	    if(angle < 0){return this.doAction(ig.ai.ACTION.MoveUp);}
	      return this.doAction(ig.ai.ACTION.MoveDown);
	    }
	  return this.doAction(ig.ai.ACTION.Rest);
